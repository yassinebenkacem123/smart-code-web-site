...
const StudentsTable = ({ 
  ...
  const { deleteStudent } = useAdminStore();
      ...
      <table className="min-w-full">
        <thead className="bg-sky-800/50">
          <tr>
            ...
            <th className="px-6 py-4 text-left text-xs font-medium text-sky-300 uppercase tracking-wider">Étudiant</th>
        ...
        <tbody className="divide-y divide-sky-500/20">
          {currentStudents.map((student, index) => (
            <tr key={student.id} className="hover:bg-sky-800/20 transition-colors">
              <td className="px-6 py-4 whitespace-nowrap text-sm text-sky-300">{indexOfFirstItem + index + 1}</td>
               
              <td className="px-6 py-4 whitespace-nowrap">
                <div className="flex items-center">
                  <div className={`${student.genre === 'Homme' ? 'bg-sky-900/30 border border-sky-500/30' : 'bg-fuchsia-900/30 border border-fuchsia-500/30'} rounded-full p-2 mr-3`}>
                    {student.genre === 'Homme' ? <IoMdMale className="text-sky-400" /> : <IoMdFemale className="text-fuchsia-500" />}
                  </div>
                  <div>
                    <div className="text-sm font-medium text-white">{student.nom} {student.prenom}</div>
                  </div>
                </div>
              </td>
              ...
              <td className="px-6 py-4 whitespace-nowrap text-sm text-white">
                {student.telephone || <span className="text-gray-500">Non renseigné</span>}
              </td>
              ...
              <td className="px-6 py-4 whitespace-nowrap text-sm text-center">
                <div className="flex justify-center gap-2">
                  <button 
                  onClick={() => {
                    navigate(`/admin-dashboard/students/details/${student.id}`)
                  }}
                  ...
                  <button 
                    onClick={() => {
                      ...
                      const body = encodeURIComponent(`Hello ${student.nom},\n\nThis is a message from the admin.`);
                      const outlookWebUrl = `https://outlook.office.com/mail/deeplink/compose?to=${student.email}&subject=${subject}&body=${body}`;
      ...
      {totalPages > 1 && (
        <div className="flex flex-col sm:flex-row items-center justify-between border-t border-sky-700/50 px-4 py-3 mt-4 gap-4">
          <div className="text-sm text-sky-300">
            ...
            <span className="ml-2">({students.length} étudiants)</span>
          </div>
