...
const EditProfessorForm = ({ 
  ...
  const { updateProfessor, professorsEorror, professorsLoading } = useAdminStore();
  const [form, setForm] = useState(professorData);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const response = await updateProfessor(professorData.id, form);
    if (response.success){
      setForm({
        nom: '',
        email: '',
      ...
      <div className="relative bg-gradient-to-br from-sky-900/30 to-blue-900/30 backdrop-blur-sm border border-sky-500/30 rounded-xl shadow-xl shadow-sky-900/30 w-full max-w-2xl">
        ...
        <form onSubmit={handleSubmit} className="p-6 md:p-8">
          <h2 className="text-2xl flex items-center gap-2 font-bold text-white mb-6"> 
            ...
            Modifier Un Professor Existant
        </h2>
