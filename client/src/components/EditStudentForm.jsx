...
const EditStudentForm = ({ 
  ...
  const { updateStudent, studentsError, studentLoading } = useAdminStore();
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const response = await updateStudent(studentData.id, form);
    if (response.success) {
      setForm({
        nom: '',
        email: '',
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            ...
            <div>
              ...
              <input
                ...
                type="date"
                value={form.date_inscription_ecole.split('T')[0]} // Format YYYY-MM-DD
                onChange={handleChange}
                required
                className="w-full bg-sky-900/30 backdrop-blur-sm border border-sky-500/30 rounded-lg py-3 px-4 text-white focus:outline-none focus:ring-2 focus:ring-sky-500"
              />
