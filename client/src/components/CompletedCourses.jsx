...
const CompletedCourses = ({completedCourses,setSlidToCompletedCourses}) => {
  ...
  return (
    <div className="bg-black text-gray-200 py-6 px-4 rounded-xl">
      <h2 className="text-2xl font-bold text-sky-300 mb-6 flex items-center">
        <MdDownloadDone className="h-6 w-6 mr-2" />
        Cours Terminés
      </h2>
      ...
      {completedCourses.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {completedCourses.map((course) => {
            ...
            return (
              <div 
                key={course.cours_id} 
                className="bg-gray-900 rounded-xl shadow-lg overflow-hidden border border-gray-800 hover:border-sky-500 transition-all duration-300 group"
              >
                <div className="relative">
                  ...
                  <div className="absolute top-3 right-3 bg-sky-600 text-white px-2 py-1 rounded-full text-xs font-medium">
                    {course.nb_chapitres_non_termines} chapitres restants
                  </div>
                </div>
                ...
                <div className="p-4 flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <div className="flex items-center text-sm text-gray-400">
                      ...
                      {course.chapitres.length} / {course.chapitres.length + course.nb_chapitres_non_termines} chapitres
                    </div>
                  </div>
        ...
        <div className="text-center py-10">
          ...
          className="mt-4 px-6 py-2 bg-sky-400 cursor-pointer text-white rounded-lg font-medium hover:bg-sky-600 transition-colors">
            Explorer les cours
          </button>
        </div>
      )}
