...
const AddAdminForm = ({ setOpenAdminForm }) => {
  const { addAdmin, adminsError, adminLoading } = useAdminStore();
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const response = await addAdmin(form);
    if (response.success) {
  ...
  return (
    <div className="fixed inset-0 bg-black/20 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="relative bg-gradient-to-br from-sky-900/30 to-blue-900/30 backdrop-blur-sm border border-sky-500/30 rounded-xl shadow-xl shadow-sky-900/30 w-full max-w-2xl">
        <button 
          onClick={() => setOpenAdminForm(false)}
        ...
        <form onSubmit={handleSubmit} className="p-6 md:p-8">
          <h2 className="text-2xl font-bold text-white mb-6">Ajouter un nouvel Admin</h2>
